...
CREATE TABLE IF NOT EXISTS public.item (
  ...
  type text NOT NULL, -- valor livre vindo da API/BAC (ex: PRODUCT, SERVICE...)
  name text NOT NULL,
  description text,
  sku text,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_item_code ON public.item(code);
CREATE INDEX IF NOT EXISTS ix_item_tenant_id ON public.item(tenant_id);
CREATE INDEX IF NOT EXISTS ix_item_tenant_type ON public.item(tenant_id, type);
CREATE UNIQUE INDEX IF NOT EXISTS ux_item_tenant_sku
  ON public.item(tenant_id, lower(sku)) WHERE sku IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS public.item_detail (
  ...
  tenant_category text, -- opcional: se precisar persistir algo da API/BAC
  ...
  CONSTRAINT ck_item_detail_duration CHECK (duration_minutes IS NULL OR duration_minutes > 0)
);

CREATE INDEX IF NOT EXISTS ix_item_detail_item_id ON public.item_detail(item_id);
CREATE INDEX IF NOT EXISTS ix_item_detail_tenant_category ON public.item_detail(tenant_category);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_item_image_code ON public.item_image(code);
CREATE INDEX IF NOT EXISTS ix_item_image_item_id ON public.item_image(item_id);
CREATE INDEX IF NOT EXISTS ix_item_image_item_sort ON public.item_image(item_id, sort_order);

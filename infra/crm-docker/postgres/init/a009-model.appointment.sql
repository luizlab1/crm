...
CREATE INDEX IF NOT EXISTS ix_appointment_scheduled_at
  ON public.appointment(scheduled_at);
...
CREATE INDEX IF NOT EXISTS ix_schedule_appointment_id
  ON public.schedule(appointment_id);
...
CREATE TABLE IF NOT EXISTS public.schedule_note (
  id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  ...
  CONSTRAINT fk_schedule_note_user
    FOREIGN KEY (user_id) REFERENCES public."user"(id) ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS ix_schedule_note_schedule_id
  ON public.schedule_note(schedule_id);

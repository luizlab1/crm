...
class UploadController(
    ...
    @PatchMapping("/{id}")
    fun update(@PathVariable id: UUID, @RequestBody request: UploadPatchRequest): ResponseEntity<UploadResponse> {
        val updated = service.update(id, request.fileType, request.entityId, request.sortOrder, request.title, request.subtitle)
    ...
    fun view(@PathVariable id: UUID): ResponseEntity<Resource> {
        ...
        return ResponseEntity.ok()
            ...
            .header(HttpHeaders.CONTENT_DISPOSITION, ContentDisposition.inline().filename(upload.fileName).build().toString())
    ...
    fun download(@PathVariable id: UUID): ResponseEntity<Resource> {
        ...
        return ResponseEntity.ok()
            .contentType(fileResolver.resolveMediaType(upload.contentType))
            .header(HttpHeaders.CONTENT_DISPOSITION, ContentDisposition.attachment().filename(upload.fileName).build().toString())
            .contentLength(resource.contentLength())
            .body(resource)
    }

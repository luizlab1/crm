...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_person_address_updated_at') THEN
    CREATE TRIGGER trg_person_address_updated_at
    BEFORE UPDATE ON public.person_address
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  END IF;
END $$;

...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_tenant_owner_updated_at') THEN
    CREATE TRIGGER trg_tenant_owner_updated_at
    BEFORE UPDATE ON public.tenant_owner
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_user_role_updated_at') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_role_permission_updated_at') THEN
